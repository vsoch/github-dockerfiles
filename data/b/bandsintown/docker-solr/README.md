...
```console
$ docker run --name my_solr -d -p 8983:8983 -t bandsintown/solr:6.6.0
```
...
```console
$ docker run --name solr_demo -d -P bandsintown/solr:6.6.0 solr-demo
```
...
```console
$ docker run -d -P bandsintown/solr:6.6.0 solr-create -c mycore
```
...
```console
$ docker run -d -P bandsintown/solr:6.6.0 solr-precreate mycore
$ docker run -d -P -v $PWD/myconfig:/myconfig bandsintown/solr:6.6.0 solr-precreate mycore /myconfig
```
...
In docker-solr you can simply pass these environment variables to the container. For example:
  ...
  docker run -d -P -e SOLR_HEAP=800m bandsintown/solr:6.6.0
...
One way of doing that is to copy the default content before running Solr:
    ...
    docker run -it -v $PWD/mysolrhome:/mysolrhome -e SOLR_HOME=/mysolrhome bandsintown/solr:6.6.0 \
...
As an added convenience, you can pass `-e INIT_SOLR_HOME=yes` to do that automatically (if SOLR_HOME is empty):
    ...
    docker run -it -v $PWD/mysolrhome:/mysolrhome -e SOLR_HOME=/mysolrhome -e INIT_SOLR_HOME=yes bandsintown/solr:6.6.0
...
See the [Can I run ZooKeeper and Solr clusters under Docker](https://github.com/bandsintown/docker-solr/blob/master/Docker-FAQ.md#can-i-run-zookeeper-and-solr-clusters-under-docker) FAQ,
...
This repository is available on [github.com/bandsintown/docker-solr](https://github.com/bandsintown/docker-solr), and the official build is on the [Docker Hub](https://hub.docker.com/bandsintown/solr/).

This repository is based on the Official Docker images `docker-solr/docker-solr`.
...
Please report issues with this docker image on this [Github project](https://github.com/bandsintown/docker-solr).
